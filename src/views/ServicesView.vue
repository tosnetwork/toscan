...
import { getServices } from "@/api/explorer";
import { useAsyncData } from "@/composables/useAsyncData";
import { compact, formatInteger, formatTos } from "@/utils/format";

const { data, loading, error, refresh } = useAsyncData(() => getServices(100));
</script>
...
<template>
  <div class="container page-container">
    ...
    <section class="surface page-surface">
      <div class="table-caption"><span>Service actors</span><span>{{ data?.length ?? 0 }} indexed</span></div>
      <LoadState :loading="loading" :error="error" :empty="!data?.length" empty-title="No service actors indexed" @retry="refresh">
        <div class="data-table-wrap"><table class="data-table"><thead><tr><th>Service</th><th>Owner</th><th>Price / call</th><th>Capacity</th><th>Revenue counter</th><th>Status</th></tr></thead><tbody>
          <tr v-for="service in data" :key="service.address">
            <td><RouterLink class="table-entity" :to="{ name: 'address', params: { address: service.address } }"><span class="avatar avatar--service">S</span><span><strong class="mono">{{ compact(service.address, 9, 7) }}</strong><small>{{ service.open_access ? "Open access" : "Restricted" }}</small></span></RouterLink></td>
