      ...
      <template v-if="data">
        ...
        <section class="surface account-details-card">
          <div class="tabs" role="tablist" aria-label="Address details">
            <button v-for="item in ([['activity', 'Activity'], ['assets', 'Assets'], ['authority', 'Authority'], ['raw', 'Raw state']] as const)" :key="item[0]" type="button" role="tab" :aria-selected="tab === item[0]" @click="tab = item[0]">{{ item[1] }}<span v-if="item[0] === 'activity'">{{ data.events.length }}</span></button>
          </div>
          ...
          <div v-else-if="tab === 'assets'" class="tab-panel" role="tabpanel">
            ...
            <div class="asset-grid">
              <article v-for="jetton in data.jettons" :key="jetton.jetton_wallet"><span class="asset-symbol">J</span><div><strong>Jetton position</strong><RouterLink class="mono" :to="{ name: 'address', params: { address: jetton.jetton_master } }">{{ compact(jetton.jetton_master) }}</RouterLink></div></article>
              <article v-for="nft in data.nfts" :key="nft.nft_item"><span class="asset-symbol asset-symbol--nft">N</span><div><strong>NFT item</strong><RouterLink class="mono" :to="{ name: 'address', params: { address: nft.nft_item } }">{{ compact(nft.nft_item) }}</RouterLink></div></article>
            </div>
