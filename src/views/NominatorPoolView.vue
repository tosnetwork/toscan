  ...
  <div class="container page-container">
    ...
    <LoadState :loading="loading" :error="error" @retry="refresh">
      <template v-if="data">
        ...
        <section v-if="data.effective_stake" class="surface stake-cap-disclosure" :class="{ 'stake-cap-disclosure--binding': data.effective_stake.surplus_earns === false }" role="note" aria-labelledby="pool-effective-stake-title">
          <div><small>Network reward constraint</small><h2 id="pool-effective-stake-title">{{ data.effective_stake.surplus_earns === false ? 'More pool capital does not always produce more rewards' : data.effective_stake.surplus_earns === true ? 'Surplus stake can earn rewards' : 'Effective-stake evidence is not available yet' }}</h2><p v-if="data.effective_stake.surplus_earns === false && data.effective_stake.effective_stake_cap">This pool’s balance is not wholly yield-bearing by definition. Elector caps reward-bearing stake for each elected validator at {{ formatTos(data.effective_stake.effective_stake_cap) }}; surplus is returned and has zero marginal yield.</p><p v-else-if="data.effective_stake.surplus_earns === true">Elector currently permits stake above the displayed cap to participate in rewards.</p><p v-else>The chain has not supplied enough completed-election evidence to state the current effective cap.</p></div>
        ...
        <p class="surface evidence-note"><strong>Evidence boundary.</strong> Historical points are observations retained by TOSCAN, not a second ledger. Network reward cycles provide context only; the chain does not currently expose a cryptographic per-pool reward attribution, so TOSCAN does not manufacture one. The effective-stake cap is Elector policy, and changing its factor is a governance action rather than something additional pool deposits can solve.</p>
      </template>
    </LoadState>
  </div>
</template>
